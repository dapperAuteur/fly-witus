...
- Monitor config: alert on any non-200, keep the check interval at or above 30s (each hit opens a
  database round trip), and have the monitor **confirm a failure with at least one re-check**. A
  warm check answers in about 0.2s, but a cold serverless start plus a suspended Neon compute waking
  up can occasionally cross the 4s budget and return a one-off 503.
